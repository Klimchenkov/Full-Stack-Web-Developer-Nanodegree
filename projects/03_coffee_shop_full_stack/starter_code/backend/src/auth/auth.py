...
AUTH0_DOMAIN = 'dev-znyh0grc.us.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'drinks'
...
def check_permissions(permission, payload):
    if 'permissions' not in payload:
        raise AuthError({
            'code': 'invalid_claims',
            'description': 'Permisiions not included in JWT.'
        }, 400)
    
    if permission not in payload['permissions']:
        raise AuthError({
            'code': 'unauthorized',
            'descrition': 'Permission not found'
        }, 403)
...
verify_decode_jwt(token) method
    ...
    it verifis the token using Auth0 /.well-known/jwks.json
...
@requires_auth(permission) decorator method
    ...
    it uses the check_permissions method to validate claims and check the requested permission
